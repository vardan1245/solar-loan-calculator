// Script to repopulate all panels with the new price_per_watt column structure
...
async function repopulateAllPanels() {
    try {

        // Clear existing data
        await pool.query('DELETE FROM panel_options');

        // Insert new panel data (price_per_watt will be calculated automatically by trigger)
        ...
        // Show all panel data with prices and price_per_watt
        const allPanelsQuery = `
            SELECT brand, model, wattage, price, price_per_watt, efficiency, warranty_years
            FROM panel_options 
